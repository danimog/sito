<template>
    <div>
        <header class="bg-red-600 text-white shadow-lg md:block">
            <div class="container mx-auto lg:grid lg:grid-cols-8 lg:gap-4 items-center lg:h-32 text-center">
                <div class="lg:col-span-2">
                    <!-- <img class="h-16" src="https://i.ibb.co/6Yxs70d/2021-10-26-23h27-03.png" alt="" /> -->
                    <span class="lg:text-2xl text-xl font-mono mx-auto uppercase font-black">Emanuele Moggia</span>
                    <br />
                    <span class="text-thin font-mono mx-auto">Candidato Consigliere regionale<br>Liguria</span>

                </div>
                <div class="lg:col-span-4 text-center">
                    <nav class="contents font-semibold text-base lg:text-lg">

                        <!-- <ul class="mx-auto flex items-center"> -->
                        <!-- <li class="p-5 xl:p-8 active">
                            <a href="">
                                <span class="font-mono">Liguri a testa alta</span>
                            </a>
                        </li> -->
                        <!-- <li class="p-5 xl:p-8"> -->
                        <button
                            class="lg:border lg:border-white rounded-full font-bold px-2 py-2 font-thin font-mono">Scarica
                            i materiali</button>
                        <!-- </li> -->
                        <!-- </ul> -->
                    </nav>
                </div>
                <div class="lg:col-span-2 lg:text-right">
                    <span class="px-2 py-2 font-thin font-mono">
                        <i class="fa-brands fa-facebook-f"></i>
                    </span>
                    <span class="px-2 py-2 font-thin font-mono">
                        <i class="fa-brands fa-instagram"></i>
                    </span>
                </div>
            </div>
        </header>
    </div>
</template>

<script setup>

</script>

<style scoped>
body {
    background-color: #e09129 !important;
}
</style>